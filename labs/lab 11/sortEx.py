...
def insertV1(L, i):
    ...
    # i is an index in list L. 
    # move that cell back through the list until it's in the right spot
    # (i.e. L, from the beginning up to the cell after the current one, 
    # is sorted, ignoring the rest of L)
...
def search(L, i, x):
    ...
    # L is the list we're looking through.
    # x is the number we're trying to insert
    # i is the index where the list might start being unsorted
    ...
    #assert allLE(L[0:output],x)
    #assert output <= i
    #assert 0 <= output and output <= i
    #assert not allLE(L[output:i],x)
...
def insertV2(L, i):
    ...
    savedValue = L[i]
    newIndex = search(L,i,savedValue)
    insertV1(L, newIndex)
...
def letterCounts(fname):
    ...
    for a in alphabet:
        calculations[a] = 0
    calculations["non-letter"] = 0
    contents = ""
    with open(fname, "r") as f:
        contents = f.read()
    for a in contents:
        calculations[letter(a)] += 1
    for a in alphabet:
        if calculations[a] == 0:
            del calculations[a]
    if calculations["non-letter"] == 0:
            del calculations["non-letter"]
    swappedDict = []
    for key in calculations:
        swappedDict += [(calculations[key],key)]
    insertSortV2(swappedDict)
    swappedDict.reverse()
    print(swappedDict)
